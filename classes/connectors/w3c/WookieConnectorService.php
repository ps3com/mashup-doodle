...
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/w3c/WookieConnectorExceptions.php");
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/w3c/WookieServerConnection.php");
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/w3c/WidgetInstances.php");
require_once("{$CFG->dirroot}/course/format/mashup/classes/connectors/w3c/Widget.php");
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/WidgetInstance.php");
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/w3c/WidgetProperties.php");
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/w3c/User.php");
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/HTTP_Response.php");
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/w3c/Logger.php");
require("{$CFG->dirroot}/course/format/mashup/classes/connectors/w3c/WookieConnectorServiceInterface.php");
...
class WookieConnectorService implements WookieConnectorServiceInterface {
	...
	/**
	 * 
	 * @param String $widgetFile - fill path on disk where the widget is located
		...
		try {
			...
			if($xmlObj instanceof SimpleXMLElement) {
				foreach($xmlObj->children() as $widget) {
				 ...
				 $iconURL = (string) $widget->icon['src'];
				 if($iconURL == '') {
						$iconURL = (string) 'http://www.oss-watch.ac.uk/images/logo2.gif';
				 }
				 $Widget = new Widget($id, $name, $description, $iconURL);
				 //$widgets[$id] = $Widget;
